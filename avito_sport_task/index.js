...
// O((n * m)^2) - второй вариант, не досмотрел, тут явно лишний квадрат (забыл матан =))
// O(2(n * m)) = O(n * m) - исправленный второй вариант (верный)

// Сложность по памяти
// создаю структуру tempResult - в худшем случае она будет содержать n * m записей
// создаю массив для вывода набора победителей winnerIds - в худшем случае он тоже может содержать всех пользователей, то есть n * m
// итого я пользуюсь двумя структурами данными, длина каждой из которых равна n * m = O(2(n * m)) = O(n * m)
